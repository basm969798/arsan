...
import { LayoutDashboard, Search, Package, Landmark, LogOut, Bell, FolderTree } from 'lucide-react';
  ...
  return (
    <div style={{ width: '260px', height: '100vh', background: '#fff', borderLeft: '1px solid #eee', display: 'flex', flexDirection: 'column', padding: '1.5rem', position: 'fixed', right: 0, top: 0, zIndex: 1000 }}>
      <div style={{ marginBottom: '2rem', textAlign: 'center', position: 'relative' }}>
        ...
        <div onClick={() => setUnreadCount(0)} style={{ position: 'absolute', left: '-5px', top: '5px', cursor: 'pointer', color: unreadCount > 0 ? '#ffc107' : '#ccc', transition: '0.3s' }}>
          <Bell size={22} />
          {unreadCount > 0 && <span style={{ position: 'absolute', top: '-5px', right: '-5px', background: '#dc3545', color: '#fff', fontSize: '10px', padding: '2px 6px', borderRadius: '10px' }}>{unreadCount}</span>}
        </div>
      </div>
      <nav style={{ flex: 1 }}>
        {menuItems.map((item) => (
